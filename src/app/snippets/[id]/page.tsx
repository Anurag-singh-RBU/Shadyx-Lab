...
function SnippetDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] font-mono text-white">
      ...
      <main className="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-12 sm:mt-0 mt-5">
        <div className="max-w-[1200px] mx-auto">
          {/* Header */}
          <div className="bg-[#121218] border border-[#ffffff0a] rounded-2xl p-6 sm:p-8 mb-6 backdrop-blur-xl">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between sm:gap-4 gap-6 mb-6">
              <div className="flex items-center gap-4">
                <div className="sm:flex items-center justify-center size-12 rounded-xl bg-[#ffffff08] p-2.5 hidden">
                  <img
                ...
                <div>
                  <h1 className="text-xl sm:text-2xl font-semibold text-white sm:mb-2 mb-6">
                    {snippet.title}
                  </h1>
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm">
                    ...
                    <div className="flex items-center gap-2 sm:mr-0 mr-2 text-gray-400">
                      <Clock className="w-4 h-4" color="white"/>
                      <span>{new Date(snippet._creationTime).toLocaleDateString()}</span>
                    </div>
                    <div className="flex items-center gap-2 text-gray-400">
                      <MessageSquare className="w-4 h-4" color="white"/>
                      <span>{comments?.length} comments</span>
                    </div>
                  </div>
                </div>
              </div>
              <div className="inline-flex items-center px-3 py-1.5 bg-teal-600 text-white rounded-lg text-sm font-medium">
                {snippet.language}
              </div>
            </div>
          </div>
