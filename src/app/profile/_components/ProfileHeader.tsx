...
function ProfileHeader({ userStats, userData, user }: ProfileHeaderProps) {
  ...
  const STATS = [
    ...
    },
    {
      ...
      metric: {
        label: "Most starred",
        value: userStats?.mostStarredLanguage.toUpperCase() ?? "None",
        icon: Trophy,
      },
    },
    {
      ...
      metric: {
        label: "Most used",
        value: userStats?.favoriteLanguage.toUpperCase() ?? "None",
        icon: TrendingUp,
      },
    },
  ];
  ...
  return (
    <div
      className="relative mb-8 bg-gradient-to-br from-[#12121a] to-[#1a1a2e] rounded-2xl sm:p-8 p-4 border
     border-gray-800/50 overflow-hidden font-mono"
    >
      <div className="absolute inset-0 bg-grid-white/[0.02] bg-[size:32px]" />
      <div className="relative flex items-center sm:gap-8 gap-4">
        <div className="relative group">
          ...
          <img
            src={user.imageUrl}
            alt="Profile"
            className="sm:w-24 sm:h-24 w-15 h-15 rounded-full border-4 border-gray-800/50 relative z-10 group-hover:scale-105 transition-transform"
          />
          {userData.isPro && (
            <div
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            <h1 className="sm:text-3xl text-2xl font-bold text-white">{userData.name}</h1>
            {userData.isPro && (
          ...
          <p className="text-gray-400 flex items-center gap-2 sm:text-md text-sm sm:ml-0 -ml-1">
            <UserIcon className="w-4 h-4 sm:block hidden" />
            {userData.email}
          </p>
        </div>
      </div>
